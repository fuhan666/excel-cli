...
- `--sheets`, `-s`：列出所有工作表并退出
- `--sheet <sheet>`：用于检索或导出的目标工作表（按名称或基于 0 的索引）
- `--peek <sheet>!<range>`, `-p <sheet>!<range>`：预览某个范围并退出（例如 `Orders!A1:F10`）
- `--cell <sheet>!<cell>`, `-c <sheet>!<cell>`：读取单个单元格并退出（例如 `Summary!B2`）
- `--format <text|json>`, `-f <text|json>`：inspect 命令的输出格式。默认：`text`
- `--json-export`, `-j`：导出为 JSON 并输出到 stdout（用于管道传输）。与 `--sheet` 组合使用时仅导出该工作表
- `--direction`, `-d`：JSON 导出中的表头方向：'h' 表示水平（顶部行），'v' 表示垂直（左侧列）。默认：'h'
- `--header-count`, `-r`：JSON 导出中的表头行数（水平方向）或列数（垂直方向）。默认：1
...
- 失败返回非零退出码
- 超出边界的范围会裁剪到实际工作表边界
- 空单元格在 text 模式下输出空字符串，在 JSON 模式下输出 `null`
